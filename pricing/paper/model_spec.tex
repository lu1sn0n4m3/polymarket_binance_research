...
has four parameters: three for the variance model ($c$, $\beta$, $\alpha$) and one for the tail shape
($\nu$). We state all assumptions explicitly, prove the key properties (QLIKE properness, variance
preservation under the $t$-overlay, and the decoupling guarantee), develop a rigorous diagnostic
framework based on conditional calibration of normalized squared returns, and specify the complete
pricer construction from raw tick data to real-time probability output. The document is self-contained
